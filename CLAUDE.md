...
- **Gemini Live model is `gemini-2.5-flash-native-audio-preview-12-2025`** (in `agent/arya.ts`), chosen because `gemini-3.1-flash-live-preview` doesn't support `generateReply()` in the LiveKit Node SDK — which Arya's agent-speaks-first greeting uses. 3.1-flash-live is a later A/B, not the default.
